...
public class PumpTubeRenderer implements BlockEntityRenderer<PumpBlockEntity> {
  ...
  public void render(PumpBlockEntity blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
    ...
    System.out.println("Tube rendering: " + tubeHeight);
    ...
    for (int i = 0; i < fullBlocks; i++) {
      renderTubeSegment(poseStack, consumer, 1.0, packedLight, packedOverlay);
      poseStack.translate(0, -1, 0);
    }

    if (partialBlock > 0) {
      renderTubeSegment(poseStack, consumer, partialBlock, packedLight, packedOverlay);
    }
  ...
  private void renderTubeSegment(PoseStack poseStack, VertexConsumer consumer, double height, int packedLight, int packedOverlay) {
    ...
    // North Face
    renderQuad(matrix, consumer, minX, minY, minZ, maxX, maxY, minZ, 0, 0, 1, 1, packedLight, packedOverlay, 0, 0, -1);
    // South Face
    renderQuad(matrix, consumer, maxX, minY, maxZ, minX, maxY, maxZ, 0, 0, 1, 1, packedLight, packedOverlay, 0, 0, 1);
    // West Face
    renderQuad(matrix, consumer, minX, minY, maxZ, minX, maxY, minZ, 0, 0, 1, 1, packedLight, packedOverlay, -1, 0, 0);
    // East Face
    renderQuad(matrix, consumer, maxX, minY, minZ, maxX, maxY, maxZ, 0, 0, 1, 1, packedLight, packedOverlay, 1, 0, 0);
    
    // 底面 (Bottom Face) - 管の先端のみ表示したい場合は条件分岐が必要だが、一旦常に表示
    if (h < 1.0f || true) {
        renderQuad(matrix, consumer, minX, minY, minZ, maxX, minY, maxZ, 0, 0, 1, 1, packedLight, packedOverlay, 0, -1, 0);
    }

    // 内側の4面をレンダリング（必要であれば）
    // 内側も描画することで、管の中が見えた時もテクスチャがあるように見える。
    // North Face (Inner)
    renderQuad(matrix, consumer, maxX, minY, minZ + 0.001f, minX, maxY, minZ + 0.001f, 0, 0, 1, 1, packedLight, packedOverlay, 0, 0, 1);
    // South Face (Inner)
    renderQuad(matrix, consumer, minX, minY, maxZ - 0.001f, maxX, maxY, maxZ - 0.001f, 0, 0, 1, 1, packedLight, packedOverlay, 0, 0, -1);
    // West Face (Inner)
    renderQuad(matrix, consumer, minX + 0.001f, minY, minZ, minX + 0.001f, maxY, maxZ, 0, 0, 1, 1, packedLight, packedOverlay, 1, 0, 0);
    // East Face (Inner)
    renderQuad(matrix, consumer, maxX - 0.001f, minY, maxZ, maxX - 0.001f, maxY, minZ, 0, 0, 1, 1, packedLight, packedOverlay, -1, 0, 0);
  }
