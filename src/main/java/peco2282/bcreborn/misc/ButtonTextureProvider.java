...
public class ButtonTextureProvider {
  ...
  private static void renderInternal(GuiGraphics graphics, ButtonSet set, ButtonType type, int x, int y) {
    Rectangle target = LARGE.get(type);
    if (target == null) {
      throw new IllegalArgumentException("Invalid button type: " + type.name() + ". Allows : " + Arrays.toString(set.validType()));
    }
    graphics.blit(BUTTONS, x, y, target.x(), target.y(), target.width(), target.height());
  }
  
  public enum ButtonType {
    ACTIVE, HOVER, HIDDEN, LOCK
  }
