                        ...
                        context -> {
                          List<IModInfo> info =
                              ModList.get().getMods().stream()
                                  .filter(i -> Objects.equals(i.getModId(), BCReborn.MODID))
                                  .toList();
                          if (info.size() > 1)
                          ...
                          if (info.isEmpty())
                            throw new SimpleCommandExceptionType(
                                    Component.literal(
                                        "Does not exist " + BCReborn.MODID + " mod in screen"))
                                .create();
                          if (context.getSource().getPlayer() == null)
                            return Command.SINGLE_SUCCESS;
                          VersionChecker.CheckResult result =
                              VersionChecker.getResult(info.getFirst());
                          if (result.status() == VersionChecker.Status.FAILED) {
                            context
                                .getSource()
                                .getPlayer()
                                .sendSystemMessage(
                                    Component.translatable("command.buildcraft.version.fail"));
                          }
                    ...
                    .executes(
                        context -> {
                          context
                              .getSource()
                              .getPlayer()
                              .sendSystemMessage(Component.literal("TODO! Implementation"));
                          return Command.SINGLE_SUCCESS;
                        })));
  }
}
